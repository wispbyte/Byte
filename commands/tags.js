...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand(sub =>
			sub.setName('send').setDescription('Send a tag')
	...
	async execute(interaction) {
		...
		if (subcommand === 'send') {
			...
			if (!tag) return interaction.reply({ content: `❌ No tag found for: ${tagName}`, flags: 'Ephemeral' });
			...
			await interaction.reply({ content: `✅ Tag '${tagName}' added!`, flags: 'Ephemeral' });
		}
		else if (subcommand === 'edit') {
			if (!tags[tagName]) return interaction.reply({ content: `❌ No tag found for: ${tagName}`, flags: 'Ephemeral' });
			tags[tagName][interaction.options.getString('field')] = interaction.options.getString('value');
			fs.writeFileSync(TAGS_FILE, JSON.stringify(tags, null, 2));
			await interaction.reply({ content: `✅ Tag '${tagName}' edited!`, flags: 'Ephemeral' });
		}
		else if (subcommand === 'list') {
			const tagList = Object.keys(tags).join(', ') || 'No tags available.';
			await interaction.reply({ content: `Tags: ${tagList}`, flags: 'Ephemeral' });
		}
		else if (subcommand === 'delete') {
			if (!tags[tagName]) return interaction.reply({ content: `❌ No tag found for: ${tagName}`, flags: 'Ephemeral' });
			delete tags[tagName];
			fs.writeFileSync(TAGS_FILE, JSON.stringify(tags, null, 2));
			await interaction.reply({ content: `✅ Tag '${tagName}' deleted!`, flags: 'Ephemeral' });
		}
	},
